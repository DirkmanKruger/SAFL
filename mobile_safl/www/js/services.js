...
angular.module('fishingLog.services', ['ngResource'])
  .constant("baseURL", "http://localhost:3000/")

  .factory('catchFactory', ['$resource', 'baseURL', function($resource, baseURL){

    return $resource(baseURL + "catches/");
  }])

  .factory('knotsFactory', ['$resource', 'baseURL', function($resource, baseURL ){
    console.log('In knotsFactory : ', baseURL + "knots");

    return $resource(baseURL + "knots/");
  }])
  .factory('speciesFactory', ['$resource', 'baseURL', function($resource, baseURL){
      ...
      $resource(baseURL + "users/login")
        .save(loginData,
          ...
          function(response){
            ...
            var message = '<div><p>' +  response.data.error.status +
              '</p><p>' + response.data.message + '</p></div>';
      ...
      $resource(baseURL + "users/register")
        .save(registerData,
          ...
          function(response){
            ...
            var message = '<div><p>' +  response.data.error.status +
              '</p><p>' + response.data.error.message + '</p></div>';
